  ...
  useEffect(() => {
    const fetchPublicBill = async () => {
      try {
        ...
        const saleTime = new Date(saleData.sale_date).getTime();
        const nowTime = Date.now();
        const diffHours = (nowTime - saleTime) / (1000 * 60 * 60);
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden">
        ...
        {isExpired ? (
          <div className="p-6 text-center space-y-4">
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-xs text-slate-600 dark:text-slate-300 text-left space-y-1">
              <div className="font-semibold text-slate-800 dark:text-slate-200">Sale Reference: {sale.sale_code}</div>
              <div>Date Issued: {new Date(sale.sale_date).toLocaleString()}</div>
              <div>Customer: {sale.customer?.name}</div>
            </div>

            <p className="text-[11px] text-slate-400">
              Need a copy? Contact Sagacious Ice Factory at {settings?.company_phone || '+94 77 123 4567'}.
            </p>
          </div>
        ) : (
